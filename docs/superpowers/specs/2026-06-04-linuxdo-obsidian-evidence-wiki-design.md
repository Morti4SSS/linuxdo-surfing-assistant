...
- 新增历史记录轻量整理命令，把 `readings_all.json` 转成热索引、source/evidence 底账、机器会话记录和有限资源候选卡。
...
- 既有阅读记录只做轻量整理：不重读网页，不把所有工具词做成人类资源卡；所有历史来源进入机器索引和 `_system` 底账，高频资源进入候选卡，人工优先看 review queue。
